...
def _dedup_nutrition_entries(metrics: dict, window_minutes: int = 90) -> dict:
    """
    Remove stale MFP meal entries.

    MyFitnessPal writes a new HealthKit record when you edit a logged meal but
    doesn't always delete the old one. This identifies and removes the earlier
    (stale) entry when ALL of the following hold:
      1. Two entries share the same data source (e.g. "MyFitnessPal")
      2. They fall within `window_minutes` of each other on the same day
      3. The LATER entry's protein, carbs, AND fat are all >= the earlier entry's
         (the later entry is a nutritional superset — consistent with adding food
         to an existing meal log)

    Cross-metric timestamp matching is used so the decision is based on the
    combined macro profile, not just a single metric.
    """
    nutrition_keys = {'dietary_energy', 'protein', 'carbohydrates', 'total_fat'}

    # Build per-timestamp macro profile
    ts_macros: dict[str, dict] = {}
    for key in nutrition_keys:
        ...
        for e in m['data']:
            ...
            if not ts:
                continue
            if ts not in ts_macros:
                ts_macros[ts] = {'source': e.get('source', ''), 'P': 0.0, 'C': 0.0, 'F': 0.0}
    ...
    stale_ts: set = set()
    sorted_ts = sorted(ts_macros.keys())

    for i, ts_i in enumerate(sorted_ts):
        if ts_i in stale_ts:
            continue
        info_i = ts_macros[ts_i]
        src_i = info_i['source']
        try:
            dt_i = datetime.strptime(ts_i[:19], '%Y-%m-%d %H:%M:%S')
        except ValueError:
            continue

        for ts_j in sorted_ts[i + 1:]:
            info_j = ts_macros[ts_j]
            if info_j['source'] != src_i:
                continue
            try:
                dt_j = datetime.strptime(ts_j[:19], '%Y-%m-%d %H:%M:%S')
            except ValueError:
                continue

            gap = (dt_j - dt_i).total_seconds() / 60
            if gap > window_minutes:
                break  # sorted — no further entry will be within window

            # Skip if neither entry has macro data (can't make a reliable decision)
            if (info_i['P'] == 0 and info_i['C'] == 0 and info_i['F'] == 0 and
                    info_j['P'] == 0 and info_j['C'] == 0 and info_j['F'] == 0):
                continue

            # j is a nutritional superset of i → i is the stale edit
            if info_j['P'] >= info_i['P'] and info_j['C'] >= info_i['C'] and info_j['F'] >= info_i['F']:
                stale_ts.add(ts_i)
                print(f"[Parser] Dedup: removed stale nutrition entry {ts_i} (superseded by {ts_j})")
                break
    ...
    for name, m in metrics.items():
        if name in nutrition_keys and m.get('data'):
            cleaned[name] = {**m, 'data': [e for e in m['data'] if e.get('date', '') not in stale_ts]}
        else:
            cleaned[name] = m
    return cleaned
...
class HealthData:
    ...
    def derived_activities(self) -> list:
        ...
        If no distance-based activities are found but workout_minutes is populated (e.g. from
        apple_exercise_time), a generic Workout entry is added — this catches strength training
        and other non-GPS workouts from Garmin that don't produce distance data.
        """
        activities = []
        ...
        # Fallback: if there are workout minutes but no distance-based activities,
        # surface them as a generic workout (covers strength training from Garmin)
        if not activities and self.workout_minutes and self.workout_minutes >= 10:
            activities.append({"name": "Workout", "duration_min": self.workout_minutes})
        return activities
    ...
    def parse(cls, raw_json: dict, target_date: Optional[str] = None) -> "HealthData":
        ...
        metrics = _dedup_nutrition_entries(metrics)
