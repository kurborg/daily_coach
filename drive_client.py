...
def load_user_configs_from_drive(folder_name: str) -> list:
    """
    Download all *.json user config files from a central configs folder in Drive.
    Files starting with '_' are treated as templates and skipped.
    Returns a list of config dicts, each with '_user_id' set to the filename stem.
    """
    service = _get_service()
    try:
        folder_id = _find_folder_id(service, folder_name)
    except FileNotFoundError:
        print(f"[Drive] Config folder '{folder_name}' not found — skipping Drive configs.")
        return []
    ...
    for f in files:
        try:
            ...
            cfg["_user_id"] = f["name"][:-5]  # strip .json
            configs.append(cfg)
            print(f"[Drive] Loaded config: {f['name']}")
        except Exception as e:
            print(f"[Drive] Warning: could not load config {f['name']}: {e}")
