...
def _build_system_prompt(cfg: dict) -> str:
    ...
    # ── Tracking gaps ─────────────────────────────────────────────────────────
    gaps = cfg.get("tracking_gaps", [])
    tracking_gaps_section = ""
    if gaps:
        gap_lines = "\n".join(f"  - {g}" for g in gaps)
        tracking_gaps_section = f"\n\nDATA NOT AVAILABLE (do not reference or flag these):\n{gap_lines}"
...
- Goal: {goal_str}{events_section}{surgery_section}{compounds_section}{tracking_gaps_section}
